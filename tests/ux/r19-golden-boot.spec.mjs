...
test.describe('R19 Golden Boot tracker', () => {
  test('Jump-to tile on Home opens the Golden Boot tracker', async ({ page }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    const tile = page.locator('[data-go="golden-boot"]');
  ...
  test('odds list renders ranked contenders incl. an elite striker', async ({ page }) => {
    const errors = [];
    ...
    // model ran in-browser on real data → an elite striker is near the top
    await expect(list).toContainText(/Mbappe|Kane|Haaland|Olmo/);
    // each row shows a boot %
